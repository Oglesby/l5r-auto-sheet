...
class SchoolService {
    constructor(skillService, ringService) {
        json.forEach((schoolJson) => {
            const school = schoolJson;

            const processSkillHolder = function(skillHolder, model, logEntries) {
                let skill = skillService[skillHolder.id];

                let options = {schoolSkill: true};
